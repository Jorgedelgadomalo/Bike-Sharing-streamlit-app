...
def main():
    ...
    tab1, tab2, tab3= st.tabs(['User Undersatnding', 'Predictive Model', 'Predict'])
    
    with tab1:
        st.title('User Demannd and Variables Undersatnding')
        st.write('In this section we want to understand the user behaviour and the relationship with the given variables.')
        ...
        with cont_demand:
            ...
            col5.metric("Total Users", f'{grouped_data["cnt"].sum():,}')
            col6.metric(f"Average Users {selected_period}",f'{grouped_data["cnt"].sum() / len(grouped_data):,.2f}')
            col7.metric("% Registered Users", f'{grouped_data["registered"].sum()/grouped_data["cnt"].sum():.2%}')
    ...
    with tab2:
        ...
        with cont_day_hour:
            ...
            col1.write(final_results)
            
            col2.subheader('Date - Hour split by type of user')
            col2.write('As we are only forecasting the total users, we need to calculate before the split of total users between Casual and Registered, based on hisotrical data.')
